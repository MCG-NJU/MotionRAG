...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
                           ...
                           guidance_rescale=0.0, ref_videos=None,
                           ref_fusion_type: Literal['mean', 'concat', 'top1', 'weight'] = None, metadata: dict = None,
                           **kwargs):
    ...
    cond['c_crossattn'] = {'image': img_emb}

    if ref_videos is not None:
        if hasattr(model, 'action_embedder') and model.action_embedder is not None:
            # ref_videos: b k f c h w
            b = ref_videos.shape[0]
            action_emb = model.action_embedder(rearrange(ref_videos, 'b k f c h w -> (b k) f c h w', b=b))
            action_emb = rearrange(action_emb, '(b k) l c -> b k l c', b=b)

            if ref_fusion_type == 'mean':
                action_emb = action_emb.mean(dim=1)

            elif ref_fusion_type == 'weight':
                distance: torch.Tensor = torch.tensor(metadata['ref_video_distance'], device=action_emb.device)
                weight = (1 - distance) / (1 - distance).sum(dim=0, keepdim=True)  # k
                action_emb = (action_emb * weight[..., None, None]).sum(dim=1)

            elif ref_fusion_type == 'concat':
                action_emb = rearrange(action_emb, 'b k l c -> b (k l) c')

            elif ref_fusion_type is None or ref_fusion_type == 'top1':
                action_emb = action_emb[:, 0]

            action_emb = model.action_proj_model(action_emb)
            cond['c_crossattn'].update({'action': action_emb})

        elif hasattr(model, 'condition_transformer') and model.condition_transformer is not None:
            batch_ = {'ref_videos': ref_videos,
                      'video':      repeat(img, 'b c h w -> b t c h w', t=ref_videos.size(2))}
            action_emb = model.condition_transformer.predict(batch_)
            cond['c_crossattn'].update({'action': action_emb})

    cond_emb = model.get_learned_conditioning(prompts)
    cond["c_crossattn"].update({'prompt': cond_emb})
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        uc['c_crossattn'].update({'prompt': uc_emb})

        uc_img_emb = model.embedder(torch.zeros_like(img))  ## b l c
        uc_img_emb = model.image_proj_model(uc_img_emb)
        uc['c_crossattn'].update({'image': uc_img_emb})
        if ref_videos is not None:
            if hasattr(model, 'action_embedder') and model.action_embedder is not None:
                uc_action_emb = model.action_embedder(torch.zeros_like(ref_videos[:, 0]))
                uc_action_emb = model.action_proj_model(uc_action_emb)
                uc['c_crossattn'].update({'action': uc_action_emb})
            elif hasattr(model, 'condition_transformer') and model.condition_transformer is not None:
                uc_act_emb = model.condition_transformer.encode_vision(torch.zeros_like(ref_videos[:, 0:1]))[:, 0]
                uc['c_crossattn'].update({'action': uc_act_emb})
        if model.model.conditioning_key == 'hybrid':
    ...
    if multiple_cond_cfg and cfg_img != 1.0:
        uc_2 = {"c_crossattn": {'prompt': uc_emb, 'image': img_emb}}
        if model.model.conditioning_key == 'hybrid':
