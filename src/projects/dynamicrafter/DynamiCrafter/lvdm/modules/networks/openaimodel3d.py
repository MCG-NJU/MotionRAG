...
class UNetModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 action_cross_attention=False,
                 action_cross_attention_scale_learnable=False,
                 temporal_action_cross_attention=False,
                 temporal_action_cross_attention_scale_learnable=False,
                 default_fs=4,
                 fs_condition=False,
                 ):
        ...
        self.temporal_self_att_only = temporal_self_att_only
        ...
        self.temporal_action_cross_attention = temporal_action_cross_attention
        self.temporal_action_cross_attention_scale_learnable = temporal_action_cross_attention_scale_learnable
        ...
        if self.addition_attention:
            self.init_attn = TimestepEmbedSequential(
                TemporalTransformer(
                    ...
                    use_checkpoint=use_checkpoint, only_self_att=self.temporal_self_att_only,
                    causal_attention=False, relative_position=use_relative_position,
                    action_cross_attention=self.temporal_action_cross_attention,
                    action_cross_attention_scale_learnable=self.temporal_action_cross_attention_scale_learnable,
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        SpatialTransformer(ch, num_heads, dim_head,
                                           ...
                                           action_cross_attention=self.action_cross_attention,
                                           action_cross_attention_scale_learnable=self.action_cross_attention_scale_learnable,
                                           )
                    )
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, **kwargs):
        ...
        ctx = {}
        if 'image' in context:
            ctx['image'] = rearrange(context['image'], 'b (t l) c -> (b t) l c', t=t)
        if 'prompt' in context:
            ctx['prompt'] = context['prompt'].repeat_interleave(repeats=t, dim=0)
        if 'action' in context:
            # ctx['action'] = rearrange(context['action'], 'b (t l) c -> (b t) l c', t=t)
            ctx['action'] = repeat(context['action'], 'b l c -> (b t) l c', t=t)
            # ctx['action'] = context['action']
        ...
        for id, module in enumerate(self.input_blocks):
            h = module(h, emb, context=ctx, batch_size=b)
            if id == 0 and self.addition_attention:
                h = self.init_attn(h, emb, context=ctx, batch_size=b)
        ...
        h = self.middle_block(h, emb, context=ctx, batch_size=b)
