...
from transformers import T5Tokenizer, T5EncoderModel, CLIPTokenizer, CLIPTextModel, VideoMAEModel
from ...common import autocast
from src.projects.condition.encoders.vit import vit_giant_patch14_224
...
class VideoMAEEmbedder(AbstractEncoder):
    ...
    def __init__(self, model: str = "MCG-NJU/videomae-base-finetuned-ssv2", freeze=True):
        super().__init__()
    ...
    def forward(self, video):
        ...
        assert video.shape[1] == 16, "Input must have 16 frames"
...
class VideoMAE2Embedder(AbstractEncoder):
    ...
    def __init__(self, model_path: str = "weights/VideoMAE2/vit_g_hybrid_pt_1200e_ssv2_ft.pth",
                 freeze=True):
        super().__init__()
        self.model = vit_giant_patch14_224(num_classes=174)

        ckpt = torch.load(model_path)['module']
    ...
    def forward(self, video):
        ...
        last_hidden_state = self.model.forward_features(pixel_values, return_embedding=True)
        return last_hidden_state
