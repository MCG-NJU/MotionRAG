...
class DDPM(pl.LightningModule):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        self.alphas_cumprod_np = alphas_cumprod
...
class LatentVisualDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True,
                 image_proj_model_trainable=True, *args, **kwargs):
    ...
    def configure_model(self) -> None:
        ...
        if self.embedder is None:
            self._init_embedder(self.img_cond_stage_config, self.freeze_embedder)

        if self.image_proj_model is None:
            self._init_img_ctx_projector(self.image_proj_stage_config, self.image_proj_model_trainable)
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False,
        ...
        if random_uncond:
            ...
            random_num = torch.ones(x.size(0),
                                    device=x.device)  ## by doning so, we can get text embedding and complete img emb for inference
        ...
        img = input_mask.to(dtype=img.dtype) * img
        ## img: b c h w
        ...
        cond["c_crossattn"] = {'prompt': prompt_imb, 'image': img_emb}
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                uc = {'prompt': uc_prompt, 'image': uc_img}
                ## hybrid case
                if isinstance(c, dict):
                    uc_hybrid = {"c_concat": [c_cat], "c_crossattn": uc}
                    uc = uc_hybrid
...
class LatentActionDiffusion(LatentVisualDiffusion):
    def __init__(self, action_cond_stage_config, action_proj_stage_config, freeze_action_embedder=True,
                 action_proj_model_trainable=True, ref_fusion_type: Literal['mean', 'concat', 'top1', 'weight'] = None,
                 *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.action_proj_model = None
        self.action_embedder = None

        self.action_cond_stage_config = action_cond_stage_config
        self.action_proj_stage_config = action_proj_stage_config
        self.freeze_action_embedder = freeze_action_embedder
        self.action_proj_model_trainable = action_proj_model_trainable
        self.ref_fusion_type = ref_fusion_type

    def configure_model(self) -> None:
        super().configure_model()
        if self.action_embedder is None:
            self.action_embedder = instantiate_from_config(self.action_cond_stage_config)

            if self.freeze_action_embedder:
                self.action_embedder.freeze()

        if self.action_proj_model is None:
            self.action_proj_model = instantiate_from_config(self.action_proj_stage_config)
            if self.action_proj_model_trainable:
                self.action_proj_model.train()
            else:
                self.action_proj_model.eval()
                for param in self.action_proj_model.parameters():
                    param.requires_grad = False

    def get_batch_input(self, batch, *args, **kwargs):
        out = super().get_batch_input(batch, *args, **kwargs)
        ref_videos = batch["ref_videos"]  # batch k_ref frame channel height width

        if random.random() < self.uncond_prob:
            action_emb = self.action_embedder(torch.zeros_like(ref_videos[:, 0])) # unconditional action embedding
        else:
            ...
            if self.ref_fusion_type == 'mean':
                action_emb = action_emb.mean(dim=1)

            elif self.ref_fusion_type == 'weight':
                distance: torch.Tensor = torch.tensor([b['ref_video_distance'] for b in batch["metadata"]],
                                                      device=action_emb.device)
                weight = (1 - distance) / (1 - distance).sum(dim=1, keepdim=True)  # b k
                action_emb = (action_emb * weight[..., None, None]).sum(dim=1)

            elif self.ref_fusion_type == 'concat':
                action_emb = rearrange(action_emb, 'b k t c -> b (k t) c')

            elif self.ref_fusion_type is None or self.ref_fusion_type == 'top1':
                action_emb = action_emb[:, 0]

        # action_emb: b t c
        action_emb = self.action_proj_model(action_emb)

        out[1]['c_crossattn']['action'] = action_emb

        return out


class LatentConditionTransformers(LatentActionDiffusion):
    def __init__(self, condition_transformer, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def get_batch_input(self, batch, *args, **kwargs):
        ...
        if random.random() < self.uncond_prob:
            action_emb = self.condition_transformer.encode_vision(torch.zeros_like(batch['ref_videos'][:, 0:1]))[:, 0]
        else:
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: dict[str, torch.Tensor] = None,
