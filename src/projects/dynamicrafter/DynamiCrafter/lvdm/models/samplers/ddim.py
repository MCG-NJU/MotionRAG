...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        alphas_cumprod = torch.tensor(self.model.alphas_cumprod_np, dtype=torch.float32, device=self.model.device)
    ...
    def sample(self,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                c = conditioning
                while not isinstance(c, torch.Tensor):
                    c = c[0] if isinstance(c, list) else list(c.values())[0]
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if isinstance(c, torch.Tensor) or isinstance(c, dict):
                x_concat = torch.cat([x] * 2, dim=0)
                t_concat = torch.cat([t] * 2, dim=0)

                cond = {}
                for k, v in c.items():
                    if isinstance(v, torch.Tensor):
                        cond[k] = torch.cat([c[k], unconditional_conditioning[k]], dim=0)
                    if isinstance(v, dict):
                        cond[k] = {kk: torch.cat([c[k][kk], unconditional_conditioning[k][kk]], dim=0) for kk in v}
                    if isinstance(v, list):
                        cond[k] = [torch.cat([vv, un_vv], dim=0) for vv, un_vv in
                                   zip(c[k], unconditional_conditioning[k])]

                for k, v in kwargs.items():
                    if isinstance(v, torch.Tensor):
                        kwargs[k] = torch.cat([v] * 2, dim=0)

                e_t = self.model.apply_model(x_concat, t_concat, cond, **kwargs)
                e_t_cond, e_t_uncond = torch.split(e_t, b, dim=0)
        ...
        if is_video:
            ...
            size = (b, 1, 1, 1)
        a_t = torch.full(size, alphas[index], device=device, dtype=torch.float32)
        a_prev = torch.full(size, alphas_prev[index], device=device, dtype=torch.float32)
        sigma_t = torch.full(size, sigmas[index], device=device, dtype=torch.float32)
        sqrt_one_minus_at = torch.full(size, sqrt_one_minus_alphas[index], device=device, dtype=torch.float32)
