...
def get_trainer_strategy(lightning_config):
    default_strategy_dict = {
        "target": "lightning.pytorch.strategies.DDPShardedStrategy"
    }
...
def load_checkpoints(model, model_cfg):
    if check_config_attribute(model_cfg, "pretrained_checkpoint"):
        ...
        if not os.path.exists(pretrained_ckpt):
            mainlogger.info(">>> Download pretrained checkpoint from huggingface hub")
            from huggingface_hub import hf_hub_download
            pretrained_ckpt = hf_hub_download(pretrained_ckpt, filename="model.ckpt")
